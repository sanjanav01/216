    ...
    if (paragraph_number <= doc->number_of_paragraphs && line_number <= lines) {
        if (line_number < lines) {
            for (i = line_number - 1; i < lines - 1; i++) {
                /*pushed lines forward until target is reached*/
                line1 = doc->paragraphs[paragraph_number - 1].lines[i];
                line2 = doc->paragraphs[paragraph_number - 1].lines[i + 1];
...
int load_document(Document *doc, char data[][MAX_STR_SIZE + 1],
    ...
    if (doc != NULL && data != NULL && data_lines >= 0) {
        ...
        for (i = 0; i < data_lines; i++) {
            if (strcmp(data[i], "") == 0) {
                /*adds paragraph if empty*/
                add_paragraph_after(doc, doc->number_of_paragraphs);
            } else {
                /*appends line if not empty*/
...
/* File loads data from a file. If an empty line is encountered, a new
paragraph is added, otherwise the line is appended to the paragraph. */
            ...
            if (doc->number_of_paragraphs < MAX_PARAGRAPHS) {
                ...
                while (fgets(line, MAX_STR_SIZE + 1, input) != NULL) {
                    if (is_line_blank(line) == SUCCESS) {
                        ...
                        line[strcspn(line, "\n")] = '\0';
                        if (append_line(doc, paras, line) == FAILURE) {
                            fclose(input);
                            return FAILURE;
                        }
                    }
...
/* Prints the paragraphs associated with the specified document to the 
specified file (overwriting the file). Return FAILURE if doc or filename
is null */
...
/* Returns SUCCESS if line is blank*/
static int is_line_blank(char line[]) {
    int values;
